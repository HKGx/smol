...
class Checker:
    ...
    def evaluate_type(
        self, t_expression: TypeExpression, scope: Scope
    ...
    def evaluate_type_expression(
        self, expression: Expression, scope: Scope[CheckerType] = None
    ) -> TypedExpression:
        ...
        match expression:
            ...
            case ArrayExpression(elements=elements):
                ...
                if element_types.count(element_types[0]) == len(element_types):
                    return TypedExpression(
                        ListType("list", element_types[0], len(element_types)),
                        expression,
                    )
            ...
            case RangeExpression(start=start, end=end, step=step):
                ...
                return TypedExpression(
                    RangeType("range", has_step=has_step), expression
                )
    ...
    def check_expr_statement(
        self, statement: ExpressionStatement, scope: Scope
    ) -> TypedStatement:
    ...
    def check_assignment_statement(
        self, statement: AssignmentStatement, scope: Scope[CheckerType]
    ) -> TypedStatement:
    ...
    def check_while_statement(
        self, statement: WhileStatement, scope: Scope
    ) -> TypedStatement:
    ...
    def check_for_statement(
        self, statement: ForStatement, scope: Scope
    ) -> TypedStatement:
    ...
    def check_function_arguments(
        self, args: list[FunctionArgument], scope: Scope
    ) -> tuple[FunctionArgumentType, ...] | None:
        ...
        for arg in args:
            ...
            if len(defined_args) > 0 and defined_args[-1].named and not is_named:
                ...
                return None
            defined_args.append(
                FunctionArgumentType(arg.name, defined_arg_type, is_named)
            )
    ...
    def check_function_definition_statement(
        self, statement: FunctionDefinitionStatement, scope: Scope, is_def: bool = False
    ) -> TypedStatement:
        name = statement.name
        args = statement.args
        body = statement.body
    ...
    def check_struct_definition_statement(
        self, statement: StructDefinitionStatement, scope: Scope, is_def: bool = False
    ) -> TypedStatement:
        ...
        for method in statement.methods:
            ...
            defined_methods.append(StructMethodType(method.name, function_type))
    ...
    def check_import_statement(
        self, statement: ImportStatement, scope: Scope, is_def: bool = False
    ) -> TypedStatement:
    ...
    def check_statement(
        self, statement: Statement, scope: Scope = None
    ) -> TypedStatement:
        if scope is None:
            scope = self.scope
        match statement:
